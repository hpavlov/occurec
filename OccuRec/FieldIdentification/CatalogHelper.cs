...
namespace OccuRec.FieldIdentification
{
    ...
    internal static class CatalogHelper
    {
        internal static List<SearchZone> GetZones(double raDeg, double deDeg, double radiusDeg)
        {
        	...
        	double aspectDec = Math.Max(Math.Abs(deFrom), Math.Abs(deTo));

			// Take care of close to singularity condition
			if (aspectDec > 89.5) aspectDec = 89.5;

            double aspect = Math.Cos(aspectDec * Math.PI / 180);
