...
namespace OccuRec.Astrometry.StarCatalogues.NOMAD
{
    public class NOMADCatalogue
    {
        ...
        {
            if (catalogMagBand == NOMADEntry.BAND_ID_R && magOutputBand == MagOutputBand.CousinsR) return measuredMag;
            if (catalogMagBand == NOMADEntry.BAND_ID_R && magOutputBand == MagOutputBand.JohnsonV) return ColourIndexTables.GetVFromRAndVR(measuredMag, vrColorIndex);

            if (catalogMagBand == NOMADEntry.BAND_ID_V && magOutputBand == MagOutputBand.CousinsR) return ColourIndexTables.GetRFromVAndVR(measuredMag, vrColorIndex);
            if (catalogMagBand == NOMADEntry.BAND_ID_V && magOutputBand == MagOutputBand.JohnsonV) return measuredMag;

            if (catalogMagBand == NOMADEntry.BAND_ID_B && magOutputBand == MagOutputBand.CousinsR) return ColourIndexTables.GetRFromBAndVR(measuredMag, vrColorIndex);
            if (catalogMagBand == NOMADEntry.BAND_ID_B && magOutputBand == MagOutputBand.JohnsonV) return ColourIndexTables.GetVFromBAndVR(measuredMag, vrColorIndex);
